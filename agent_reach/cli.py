...
def main():
    ...
    # ── check-update ──
    # ── mcp ──
    p_mcp = sub.add_parser("mcp", help="Run MCP server (stdio) — expose platforms as MCP tools")
...
def _cmd_install(args):
    ...
    if args.proxy:
        if dry_run:
            ...
            print(f"✅ Proxy saved (used when Agent accesses restricted networks)")
    ...
    if requested_channels and not dry_run and not safe_mode:
        ...
        if env == "server" and "opencli" in requested_channels:
            ...
            print("  -- OpenCLI requires desktop environment + Chrome, skipping on server")
    ...
    if env == "server":
        print()
        print("Tip: Some platforms have risk control on server IPs.")
        print("   Reddit requires login (rdt-cli + Cookie, see doctor output), and proxy for China mainland networks.")
        print("   Save proxy for Agent use: agent-reach configure proxy http://user:pass@ip:port")
    ...
    if not dry_run:
        ...
        # Star reminder
        print()
        print("If Agent Reach helped you, give a Star to help others discover it:")
        print("   https://github.com/Panniantong/Agent-Reach")
        print("   Just one second, means a lot to an indie developer. Thanks!")
    else:
        print()
...
def _install_xhs_deps():
    ...
    if _detect_environment() == "server":
        print("  Server environment recommends xiaohongshu-mcp (headless browser, QR login):")
        print("    1. Download binary: https://github.com/xpzouying/xiaohongshu-mcp/releases")
        print("       (recommend placing under ~/.agent-reach/tools/)")
        print("    2. Start the service (first run downloads ~150MB browser, please wait)")
        print("    3. After QR login, connect: mcporter config add xiaohongshu http://localhost:18060/mcp")
        print("    4. Verify: agent-reach doctor")
        return

    _install_opencli_deps()
    if shutil.which("xhs"):
        print("  ✅ Detected existing xhs-cli, will remain available as fallback backend")
...
def _install_opencli_deps():
    ...
    if st.installed and not st.broken:
        ...
        print("  Final step (manual, Chrome security restriction): install browser extension")
...
def _install_reddit_deps():
    ...
    if _detect_environment() != "server":
        _install_opencli_deps()
        print("  Reddit uses OpenCLI (works if logged into reddit.com in browser)")
        import shutil
        if shutil.which("rdt"):
            print("  ✅ Detected existing rdt-cli, will remain available as fallback backend")
        return
...
def _cmd_mcp():
    ...
    from agent_reach.integrations.mcp_server import main as mcp_main
    mcp_main()
...
def _cmd_configure(args):
    ...
    if args.key == "proxy":
        ...
        print("✅ Proxy saved (for Agent to set HTTP_PROXY/HTTPS_PROXY when accessing Reddit/Twitter etc. via proxy)")
        print("  Note: Bilibili uses bili-cli, no proxy needed within China.")
...
def _cmd_setup():
            ...
            if "exa" in r.stdout.lower():
                ...
                if setup_now in ("", "y", "yes"):
                    ...
                    if add_r.returncode == 0:
                        ...
                        print("  [!] Auto-configuration failed, please do it manually:")
                        print("     mcporter config add exa https://mcp.exa.ai/mcp")
        except Exception:
            print("  [!] Could not check Exa config, please configure manually:")
            print("     mcporter config add exa https://mcp.exa.ai/mcp")
        print()

    # Step 2: GitHub token
    print("[Optional] GitHub Token — Increase API limit")
    print("  Without token: 60 req/hour | With token: 5000 req/hour")
    ...
    if current:
        ...
        if key:
            ...
            print("  ✅ GitHub API boosted to 5000 req/hour!")
        else:
            print("  Skipped. Public API works too")
    print()

    # Step 3: Reddit — rdt-cli
    print("[Info] Reddit — Login required (no zero-config path). Desktop: OpenCLI recommended; or rdt-cli:")
    print(f"  Install: pipx install '{_RDT_GIT_SOURCE}'")
    print("  Then run: rdt login (login to reddit.com in browser first)")
    print()
...
_UPDATE_INSTRUCTIONS = (
    "Update method (recommended — copy this line to your AI Agent for a full update of package + upstream tools + skill):\n"
    "  Help me update Agent Reach: https://raw.githubusercontent.com/Panniantong/agent-reach/main/docs/update.md\n"
    "Package-only update (without upstream tools and skill):\n"
...
def _cmd_check_update():
    ...
    if err:
        print(f"[!] Cannot check for updates ({_update_error_text(err)}, retried {attempts} times)")
        return "error"
    ...
    if release_err == "rate_limit":
        print("[!] Cannot check for updates (GitHub API rate limit, please retry later)")
        return "error"
    ...
    if err2:
        print(f"[!] Cannot check for updates ({_update_error_text(err2)}, retried {attempts + attempts2} times)")
        return "error"
    ...
    print(f"[!] Cannot check for updates (GitHub returned {resp2.status_code})")
    return "error"
    ...
    if update_available:
        ...
        if release_body:
            for line in release_body.strip().split("\n")[:10]:
                print(f"    {line}")
        print("  Update (send one line to your Agent for full update):")
