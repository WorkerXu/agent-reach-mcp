...
def check_all(config: Config) -> Dict[str, dict]:
    ...
    for ch in get_all_channels():
        try:
            ...
            status, message, active = "error", f"Health check failed: {e}", None
        results[ch.name] = {
            "status": status,
            "name": ch.description,
            "message": message,
...
def format_report(results: Dict[str, dict]) -> str:
    ...
    if all_inactive:
        ...
        lines.append(
            ...
            "tell your Agent 'help me install XXX' to set up"
        )
    ...
    if config_path.exists() and sys.platform != "win32":
        try:
            ...
            if mode & (stat.S_IRGRP | stat.S_IROTH):
                lines.append("")
                lines.append(
                    "[bold red][!]  Security warning: config.yaml permissions too permissive (other users can read)[/bold red]"
                )
