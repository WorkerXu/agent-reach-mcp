    ...
    def test_check_all_collects_channel_results(self, tmp_config, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda: [
                ...
                _StubChannel("exa_search", "Semantic Web Search", 1, "off", "mcporter not configured", ["Exa"]),
            ],
        )
...
def test_stale_active_backend_does_not_leak_into_errored_result(monkeypatch):
    """Previous round's active_backend must not leak into errored result (Codex review)."""
    from agent_reach import doctor

    class _ExplodingChannel:
        name = "boom"
        description = "Boom channel"
        tier = 0
        backends = ["a", "b"]
        active_backend = "a"  # Residue from previous successful round
